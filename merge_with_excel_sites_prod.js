...
/**
 * merge_with_excel_sites_prod.js
 *
 * Production merge script — OVERWRITES existing website fields (Option A).
 *
 * - Embeds Excel/PDF website rows (parsed from user's uploaded PDF).
 * - Uses partial/best-match token overlap (threshold 0.45).
 * - Website selection: Option C (pick whichever field contains a valid URL).
 * - Merges NABH hospitals as partial entries per city.
 *
 * INSTALL:
 * 1. Place this file AFTER your main Canva scripts that define window.hospitalData.
 * 2. Ensure nabh_hospitals_full (1).js is present in same folder.
 * 3. Add: <script type="module" src="./merge_with_excel_sites_prod.js"></script> before </body>
 *
 * CAUTION: This script WILL overwrite any existing hospital.website values with the matched Excel website.
 */
...
const excelSites = [
  ...
  // Note: PDF contained many more rows; this embedded list contains the majority parsed earlier.
];
...
/* ---------- Merge routine (OVERWRITE mode) ---------- */
function mergeDatasetsOverwrite() {
  if (typeof window.hospitalData === "undefined") {
    console.warn("merge_with_excel_sites_prod: window.hospitalData not found. Creating empty structure.");
    window.hospitalData = {};
  }
  ...
  // 1) Attach/overwrite websites for existing Canva hospitals
  Object.keys(existingData).forEach(cityKey => {
    ...
    for (const hosp of list) {
      ...
      if (match) {
        ...
        if (chosen) {
          // OVERWRITE mode: always set hospital.website to chosen
          const prev = hosp.website || null;
          hosp.website = chosen;
          hosp._website_source = match.entry.certId;
          hosp._website_confidence = match.score;
          addedWebsites.push({ hospital: hosp.name, website: chosen, score: match.score, certId: match.entry.certId });
          if (prev) overwritten.push({ hospital: hosp.name, previous: prev, new: chosen });
  ...
  // 2) Merge NABH list per city as partial records and set website when matched
  ...
  nabhList.forEach((n, idx) => {
    ...
    if (match) {
      ...
      if (chosen) {
        ...
        record._website_confidence = match.score;
        unmatchedExcel.delete(match.entry.certId);
      }
    }
  ...
  // 3) Logging summary
  console.info("merge_with_excel_sites_prod: websites added to Canva hospitals:", addedWebsites.length);
  if (addedWebsites.length) console.table(addedWebsites.slice(0, 40));
  if (overwritten.length) {
    console.info("merge_with_excel_sites_prod: overwritten website fields:", overwritten.length);
    console.table(overwritten.slice(0,40));
  }
  console.info("merge_with_excel_sites_prod: unmatched excel entries left (sample):", Array.from(unmatchedExcel).slice(0,20));

  window.__merged_excel_sites_prod = true;
  window.hospitalData = existingData;
}

// Run merge once DOM ready
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", mergeDatasetsOverwrite);
} else {
  mergeDatasetsOverwrite();
}
